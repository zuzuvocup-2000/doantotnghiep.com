...
class Catalogue extends FrontendController{
   ...
   protected $provinceRepository;
	...
	public function __construct(){
      $this->data = [];
      $this->data['module'] = 'project_catalogue';
   ...
   public function projectByCity($cityId = '', $page = 1){
      $detailCity = $this->AutoloadModel->_get_where([
        'select' => '*',
        'table' => 'vn_province',
        'order_by' => 'order desc',
        'where' => [
      ...
      $project = $this->projectService->projectByCity($detailCity, $page);

      $canonical = write_url($project['canonical']);
      $general = $this->general;
      $city = $this->array->converProvinceArray($this->provinceRepository->allProvince());

      $meta_title = 'Cần Bán Bất động sản tại '.$detailCity['name'];
      $meta_description = 'Cần Bán Bất động sản tại '.$detailCity['name'].' Cập nhật thông tin mới nhất các dự án tại '.$detailCity['name'];
      $province = $this->AutoloadModel->_get_where([
        'select' => '*, (SELECT COUNT(id) FROM project WHERE vn_province.provinceid = project.city_id) as project_count',
        'table' => 'vn_province',
        'order_by' => 'order desc'
     ], TRUE);

      $projectTypeList = $this->AutoloadModel->_get_where([
      'select' => '*',
      'table' => 'project_type',
      ], TRUE);
      ...
      $city = $this->array->converProvinceArray($this->provinceRepository->allProvince());
      $template = convertUrl('frontend.project.catalogue.projectByCity');
      return view(convertUrl('frontend.homepage.layout.home'), compact(
         'general','city','project','template','meta_title','meta_description','canonical','province', 'city','detailCity', 'projectTypeList'
         )
      );
   }

   public function findProjectByType($projectType = 0, $page = 1){

      $typeid = ($this->request->getGet('project_type_id')) ? $this->request->getGet('project_type_id') : $projectType;
      $cityId = ($this->request->getGet('city_id')) ? $this->request->getGet('city_id') : 0;
      $districtid = ($this->request->getGet('district_id')) ? $this->request->getGet('district_id') : 0;
      ...
      $detailDistrict = $this->AutoloadModel->_get_where([
        'select' => '*',
        'table' => 'vn_district',
        'where' => [
      ...
      $projectTypeDetail = $this->projectType->findById($typeid, $this->language);
      $project = $this->projectService->findProjectByType($projectTypeDetail, $cityId, $districtid ,$page);

      $canonical = write_url($project['canonical']);
      $general = $this->general;
      $city = $this->array->converProvinceArray($this->provinceRepository->allProvince());

      $meta_title = 'Cần Bán Bất động sản tại '.$projectTypeDetail['title'];
      $meta_description = 'Cần Bán Bất động sản tại '.$projectTypeDetail['title'].' Cập nhật thông tin mới nhất các dự án tại '.$projectTypeDetail['title'];
      $province = $this->AutoloadModel->_get_where([
        'select' => '*, (SELECT COUNT(id) FROM project WHERE vn_province.provinceid = project.city_id) as project_count',
        'table' => 'vn_province',
        'order_by' => 'order desc'
      ...
      $city = $this->array->converProvinceArray($this->provinceRepository->allProvince());
      $template = convertUrl('frontend.project.catalogue.findProjectByType');
      return view(convertUrl('frontend.homepage.layout.home'), compact(
         'general','city','project','template','meta_title','meta_description','canonical','province', 'projectTypeDetail', 'city','projectTypeList','detailCity','detailDistrict'
         )
      );
   }
	...
	public function index($id = 0, $page = 1){
        ...
        $config['total_rows'] = $this->AutoloadModel->_get_where([
            ...
            'join' => [
                ...
                [
                    'project_translate as tb3','tb1.id = tb3.objectid AND tb3.module = "project" AND tb3.language = \''.$this->currentLanguage().'\' ','inner'
                ],
                [
                    'project_translate as tb4','tb1.catalogueid = tb4.objectid AND tb4.module = "project_catalogue" AND tb4.language = \''.$this->currentLanguage().'\' ','inner'
                ]
            ],
            'count' => TRUE
        ]);
        ...
        if($config['total_rows'] > 0){
            ...
            $this->data['articleList'] = $this->AutoloadModel->_get_where([
                'select' => 'tb1.id,tb1.created_at,tb1.viewed, tb1.image, tb3.title, tb3.canonical, tb3.meta_title, tb3.meta_description, tb3.viewed, tb3.description, tb3.content, tb4.title as cat_title, tb1.catalogueid,  tb4.title as cat_title, tb4.canonical as cat_canonical, tb4.description as cat_description, tb4.template as cat_template, tb1.address, (SELECT name FROM vn_district WHERE districtid = tb1.district_id) as district, (SELECT name FROM vn_province WHERE provinceid  = tb1.city_id) as city',
                'table' => $module_extract[0].' as tb1',
                'where' => [
                    'tb1.deleted_at' => 0,
                    'tb1.publish' => 1
       ...
       $this->data['projectType'] = $this->AutoloadModel->_get_where([
         'select' => '*',
         'table' => 'project_type',
      ], TRUE);
      ...
      $this->data['projectTypeList'] = $this->AutoloadModel->_get_where([
        ...
        if(isset($this->data['detailCatalogue']['template']) && $this->data['detailCatalogue']['template'] != ''){
            $this->data['template'] = 'frontend/project/catalogue/'.$this->data['detailCatalogue']['template'];
        }else{
            $this->data['template'] = 'frontend/project/catalogue/index';
        }
    ...
    public function condition_catalogue($catalogueid = 0){
        ...
        if($catalogueid > 0){
            $catalogue = $this->AutoloadModel->_get_where([
                ...
                'join' =>  [
                    [
                        'project_translate as tb3','tb1.id = tb3.objectid AND tb3.language = \''.$this->currentLanguage().'\' AND tb3.module = "project_catalogue"','inner'
                    ],
                ],
            ...
            foreach ($listObject as $key => $value) {
                ...
                $html = $html.'<div class="grid-item col-sm-6 col-lg-4 lifestyle shopping winter-sale">';
                    $html = $html.'<article class="post">';
                        $html = $html.'<figure class="post-project designs-project">';
